...
# Restrict to grape root samples
grape_root_samples <- colnames(read.table("data/diversity_files/bacteria/diversity_grape/bray_curtis_distance_matrix_exported/distance-matrix.tsv",
                                          header=TRUE, sep="\t", stringsAsFactors = FALSE, row.names=1))

bacteria_meta <- read.table("data/metadata/root_depth_bacteria_metadata.tsv",
                            header=TRUE, sep="\t", stringsAsFactors = FALSE, row.names=1)

bacteria_meta <- bacteria_meta[grape_root_samples, ]
...
bacteria_ASVs <- bacteria_ASVs[, grape_root_samples]
...
root_depth_aldex_kw <- read_tsv("data/ALDEx2_out/bacteria_root_depth_aldex_kw.txt")
root_depth_aldex_kw <- root_depth_aldex_kw %>% mutate(factor="root depth") %>% select("bacteria genus", factor, kw.ep:glm.eBH)

rootstock_aldex_kw <- read_tsv("data/ALDEx2_out/bacteria_rootstock_aldex_kw.txt")
...
write.table(aldex_bacteria_results, "data/ALDEx2_out/aldex_bacteria_results.csv", sep=",", quote=F, row.names = F)
...
write.table(aldex_bacteria_results_sig, "data/ALDEx2_out/aldex_bacteria_results_sig.csv", sep=",", quote=F, row.names = F)

#Look at distributions for significant genera
#Convert to relative abundances first
bacteria_asv_abun <- data.frame(sweep(bacteria_asv_abun_genus_sum[,2:ncol(bacteria_asv_abun_genus_sum)],
                                      2,
                                      colSums(bacteria_asv_abun_genus_sum[,2:ncol(bacteria_asv_abun_genus_sum)]),
                                      '/')) * 100
...
pdf("figures/rootstock_bacteria_distributions.pdf", width=6.5, height=8,family="Arial")
...
pdf("figures/rootstock_bacteria_distributions_vine.pdf", width=7, height=10,family="Arial")
